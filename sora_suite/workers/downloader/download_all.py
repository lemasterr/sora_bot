    ...
    # === НОВОЕ: дефолты путей от корня проекта ===
    ...
    def jitter(a=0.08, b=0.25): time.sleep(random.uniform(a, b))
    def long_jitter(a=0.8, b=1.8): time.sleep(random.uniform(a, b))
    ...
    def write_cursor(path: str, idx: int):
        try:
    ...
    def next_custom_title() -> Optional[str]:
        """Берём следующее имя из titles.txt. Если после очистки пусто — считаем, что имени нет (курсор всё равно двигаем)."""
        if not TITLES_FILE:
            return None
    ...
    def find_or_open_drafts_page(context):
        for p in context.pages:
            if p.url.startswith(DRAFTS_URL):
                return p
    ...
    def main():
        ...
        with sync_playwright() as p:
            try:
                ...
                if not contexts:
                    raise RuntimeError("Нет контекстов Chrome. Запусти Chrome с --remote-debugging-port=9222 и сессией Sora.")
                context = contexts[0]
                ...
                for k, i in enumerate(indices, 1):
                    ...
                    if not open_card(page, i):
                        print("[!] Карточка исчезла при открытии — пропускаю."); continue
                    long_jitter()
                    try:
                        open_kebab_menu(page)
                    except PwTimeout:
                        print("[!] Не нашёл меню «три точки» — пропускаю."); go_back_to_drafts(page); continue
                    try:
