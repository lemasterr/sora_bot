...
- В разделе **Настройки → FFmpeg → Автодетект водяного знака** включите переключатель и укажите путь к шаблону PNG/JPG.
- Порог `threshold` и количество кадров `frames` задают уверенность совпадения. Если знак «прыгает» каждые ~30 секунд, увеличьте `frames` (например, до 8–10), чтобы выборка покрыла разные моменты ролика.
- Опция `downscale` (0–1 — коэффициент, >1 — максимальная сторона) уменьшает кадры перед анализом, ускоряя поиск на 4K-материалах.
- При блюре Sora Suite равномерно выбирает кадры, прогоняет `cv2.matchTemplate`, группирует найденные позиции по времени и строит сегменты. Для каждого сегмента пересчитываются координаты пресета и формируется `delogo` с `enable='between(t,start,end)'`, поэтому перемещающийся водяной знак скрывается точно в нужный момент.
- Если уверенных совпадений нет, приложение пишет причину в лог, историю и Telegram и автоматически возвращается к статическим зонам, чтобы FFmpeg не падал.
