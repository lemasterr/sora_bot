...
## Скачивание роликов из Sora
- Downloader теперь умеет работать прямо из карточки, листая ленту как в TikTok: он скачивает открытое видео, прокручивает страницу вниз один раз, ждёт смену URL и появления правой панели, а затем качает следующий ролик.
- Откройте нужную карточку заранее или оставьте список черновиков открытым — скрипт сам кликнет по первой карточке и продолжит листать вниз.
- Запуск из корня проекта: `python sora_suite/workers/downloader/download_all.py`. Переменные окружения `MAX_VIDEOS`, `DOWNLOAD_DIR` и `TITLES_FILE` работают так же, как раньше (0 в `MAX_VIDEOS` — без ограничения).
- Если URL меняется с задержкой, downloader делает паузы и несколько прокруток, чтобы не уйти дальше, чем нужно. Когда карточку открыть не удаётся, включается прежний режим: сканирование списка и последовательное открытие ссылок.
