...
- Раздельный редактор image-промптов с быстрым запуском генерации картинок и кнопкой открытия итоговой папки.
...
2. На вкладке **Задачи** отметьте нужные этапы, нажмите «Старт выбранного» — рядом доступна отдельная кнопка запуска без повторного выбора. Для автогена картинок теперь есть собственная кнопка «Генерация картинок (Google)» и быстрый переход к папке `generated_images/`.
