...
Приложение целиком переехало на Electron + React с тёмным административным дизайном. Старый PyQt6 UI удалён: при запуске бэкенд больше не создаёт Qt-окна, а отдаёт собранный фронтенд.
...
- Production/просмотр билда:
  ...
  ```
  Скрипт поднимает локальный сервер на свободном порту (по умолчанию 5173) и открывает вкладку с новым дизайном.
- Разработка:
  ```bash
  npm run dev  # в другой вкладке терминала
  python -m sora_suite.app --dev
  ```
  `--dev` просто открывает уже работающий Vite dev server.
