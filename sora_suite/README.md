...
- Если видите однотонный синий фон, удалите старый `frontend/dist` и выполните `npm run build` заново — пути ассетов теперь
  относительные и корректно работают при загрузке из `file://` в Electron.
