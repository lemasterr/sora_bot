...
- Порог `threshold` и количество кадров `frames` задают уверенность совпадения. Если знак «прыгает» каждые ~30 секунд, увеличьте `frames` (например, до 8–10), чтобы выборка покрыла разные моменты ролика. `downscale` (0–1 — коэффициент, >1 — максимальная сторона) уменьшает кадры перед анализом и ускоряет поиск на 4K-материалах.
