...
3. В верхней панели есть кнопка «Обновить из GitHub»: она запускает `git fetch`/`git pull` без перехода в настройки.
...
- Кнопки «Проверить обновления» и «Обновить из GitHub» вызывают `git fetch`/`git pull`; дублирующая кнопка находится и в верхней панели главного окна.
...
- Активный пресет и таблица зон сохраняются автоматически: выберите нужный пресет или отредактируйте координаты и запускайте блюр — при следующем запуске приложения выбранный шаблон останется активным.
