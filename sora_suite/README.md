...
- Менеджер промптов с автообнаружением профилей Chrome, файлами `prompts_<slug>.txt` и списком использованных строк.
- Раздельный редактор image-промптов: строки обрабатываются по порядку, поддерживают JSON с `prompts`/`count`/`video_prompt`,
  а кнопка генерации запускает отдельный прогон Google AI Studio и обновляет `generated_images/manifest.json` без участия Sora.
  Рядом расположена кнопка для мгновенного открытия итоговой папки.
...
2. На вкладке **Задачи** отметьте нужные этапы, нажмите «Старт выбранного» — рядом доступна отдельная кнопка запуска без повторного выбора. Кнопка «Генерация картинок (Google)» запускает отдельный прогон Google AI Studio, сохраняет результаты в `generated_images/` и фиксирует их в manifest, чтобы перед отправкой в Sora можно было просмотреть и отобрать изображения.
3. В верхней панели есть кнопка «Обновить из GitHub»: она запускает `git fetch`/`git pull` без перехода в настройки (если проект запущен из git-репозитория).
4. Карточка «Сейчас» показывает текущий шаг, цветовое состояние и таймер; история справа/слева сворачивается по чекбоксу, плотность записей переключается между «компакт» и «стандарт».
5. Линия статистики под статусом подсвечивает количество файлов в RAW/BLURRED/MERGED и размер очередей YouTube/TikTok.
...
- Сплиттеры позволяют независимо растягивать редактор и историю использованных промптов.
- Каждый профиль сохраняет логи отправок отдельно; кнопка автосканирования находит каталоги Chrome для Windows/macOS/Linux.

### Промпты картинок
- Отдельная вкладка с текстовым редактором `image_prompts.txt`. Строки применяются по порядку к основному списку промптов.
- Поддерживаются JSON-строки с полями `prompt(s)`, `count` и `video_prompt`, либо простые записи, разделённые `||` для нескольких вариантов.
- Поле `video_prompt` позволяет задать конечный текст для Sora на каждую картинку. Если в настройках отключено прикрепление изображений, воркер сохранит файлы, но пропустит их загрузку.
- После генерации воркер фиксирует соответствие строки `image_prompts.txt` и файлов в `generated_images/manifest.json`, чтобы следующий запуск видео-автогена мог автоматически прикрепить проверенные изображения или использовать только текст.
- Перед запуском кнопкой «Генерация картинок (Google)» файл автоматически сохраняется; рядом доступна кнопка открытия каталога `generated_images/`.
- Сгенерированные изображения автоматически переименовываются в последовательные номера (`1.jpg`, `2.jpg`, …) по аналогии с загрузчиком видео, поэтому в каталоге `generated_images/` сразу видно порядок и не нужно вручную исправлять имена.
- При последующем запуске видео-автогена воркер сам нажмёт кнопку добавления медиа (`+`), выберет свежие файлы из `generated_images/` и подставит соответствующий `video_prompt`.

## Работа с Chrome и портами CDP
- Поле **Chrome CDP порт** в настройках хранится в `chrome.cdp_port` и используется кнопкой «Открыть Chrome (CDP)». По умолчанию применяется `9222`.
- При запуске приложение проверяет, свободен ли порт и доступен ли Chrome DevTools Protocol. Если браузер уже поднят на этом порту, статус сообщает «Chrome уже поднят».
- Чтобы открыть второе окно для другого профиля, выберите новый порт (например, `9333`), нажмите «Применить настройки», затем воспользуйтесь кнопкой «Открыть Chrome (CDP)». Первый экземпляр можно оставить на 9222: Sora Suite не мешает держать несколько окон, пока порты уникальны.
- Каждый профиль хранит свой `user_data_dir` и `profile_directory`. При смене активного профиля редактор промптов переключает файл `prompts_<slug>.txt`, поэтому чередовать окна удобно: создайте отдельный порт, выберите профиль, запустите Chrome и работайте со своим набором промптов.
- Если нужно временно подключиться к уже открытому окну, просто выставьте соответствующий порт: приложение перепроверит соединение и не станет поднимать новый экземпляр.
...
### Настройки
- Вкладки: *Каталоги*, *Chrome*, *FFmpeg*, *YouTube*, *TikTok*, *Telegram*, *Генерация картинок*, *Обслуживание*, *Документация*.
...
- Пути к папкам (downloads/blurred/merged/generated_images, источники YouTube/TikTok и т.д.) нормализуются относительно папки `sora_suite`, поэтому downloader, блюр и другие шаги работают с одними и теми же каталогами рядом с `app_config.yaml`, а не в соседних директориях.
...
- Раздел «Генерация картинок» содержит все параметры Google AI Studio, а также чекбоксы для включения генерации и для прикрепления полученных изображений к заявке в Sora.
...
- Кнопки «Проверить обновления» и «Обновить из GitHub» вызывают `git fetch`/`git pull`; дублирующая кнопка находится и в верхней панели главного окна. Если приложение запущено из распакованной сборки (`.exe`, `.app`, `.dmg`), где нет каталога `.git`, кнопки отключены и выводят подсказку с инструкциями.
...
## Блюр водяного знака
- Исходники ищутся в папке, указанной в `Настройки → Каталоги → Источник для BLUR`. Если каталог недоступен, приложение автоматически подхватывает файлы из общего `downloads/` рядом с `app_config.yaml`. Это позволяет просто скопировать ролики в `sora_suite/downloads` и запускать блюр без изменения настроек.
- Поддерживаемые расширения: `mp4`, `mov`, `m4v`, `webm`, `mkv`. Каждое имя файла берётся только один раз — если ролик уже лежит в целевом каталоге, он не будет продублирован.
- Перед запуском убедитесь, что в активном пресете есть прямоугольники. Настроить их можно через кнопку «Предпросмотр» — OpenCV покажет видео с полупрозрачной маской.
- FFmpeg работает в несколько потоков (`blur_threads`). Статусы и ошибки (например, отсутствие бинаря) появляются в карточке «Сейчас», истории и, при включённом Telegram, в чате.
- Активный пресет и таблица зон сохраняются автоматически: выберите нужный пресет или отредактируйте координаты и запускайте блюр — при следующем запуске приложения выбранный шаблон останется активным. Старые конфиги (где зоны были сохранены в другом формате) автоматически конвертируются и подхватываются предпросмотром.

## Автоскачивание драфтов
- Downloader подключается к открытому окну Sora через Chrome DevTools Protocol и теперь прокручивает страницу, чтобы собрать столько карточек, сколько указано в поле «Сколько скачивать» (0 = все).
- Скрипт работает с первыми `N` карточками в порядке сверху вниз: после скачивания он возвращается к началу ленты, подгружает следующую порцию и не перескакивает на старые ролики внизу списка.
- Ограничение на количество загрузок сохраняется в настройках и используется как во вкладке «Скачка», так и при запуске сценария из главного экрана.
...
1. Нажмите кнопку «Обновить из GitHub» в верхней панели или во вкладке *Настройки → Обслуживание*. Приложение покажет в карточке «Сейчас», что запускается проверка.
2. Внутри выполняется `git fetch` и `git pull` внутри каталога `sora_suite`. Если локальные изменения мешают, событие попадёт в историю с текстом ошибки, а файлы останутся нетронутыми.
3. После успешного обновления кнопка подсветит сообщение «Репозиторий обновлён», а историю дополнит записью с новой ревизией.
4. Для сценариев без графики используйте `python scripts/self_update.py` — скрипт повторяет те же шаги и полезен на CI.
5. **Если приложение запускается из готовой сборки или архивной версии без `.git`:** кнопки обновления будут выключены. Чтобы получить новую версию, скачайте свежий архив/инсталлятор, распакуйте его поверх существующей папки (или в новый каталог) и перенесите `app_config.yaml`, `downloads/`, `blurred/`, `merged/`, `generated_images/` и другие рабочие папки.
...
Функция параллельной подачи промптов отключена: редактор хранит отдельные файлы `prompts_<slug>.txt` на профиль, но запуск дополнительных окон больше не доступен из интерфейса. Используйте основной автоген или запускайте собственные скрипты Playwright вручную при необходимости.
