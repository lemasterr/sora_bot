...
На странице **Настройки → FFmpeg** появилась кнопка «Автодетект водяного знака». Алгоритм берёт последний ролик из папки BLUR, извлекает кадр через FFmpeg и анализирует края при помощи Pillow. Найденные координаты автоматически заполняются в активный пресет (9:16 или 16:9), а при необходимости значения можно подредактировать вручную и сохранить. Требование — установленный `Pillow` (он уже добавлен в `requirements.txt`).
