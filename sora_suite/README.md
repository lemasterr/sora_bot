...
### Предпросмотр видео (OpenCV)

Мини-плеер больше не зависит от QtMultimedia — предпросмотр реализован поверх `opencv-python`, который входит в `requirements.txt`.
Если при установке пакета появляются ошибки или Python сообщает, что колёса отсутствуют, выполните следующие шаги:
...
* **macOS на Apple Silicon (M1/M2/M3):** убедитесь, что активирован arm64-интерпретатор Python. Если вы устанавливали Python через
  Homebrew, команда `arch -arm64 python3 -m pip install opencv-python` подтянет корректные бинарники.
* **Windows:** устанавливайте зависимости из «x64 Native Tools Command Prompt» или обычного PowerShell — колёса `opencv-python`
  имеют поддержку Windows 10/11 без дополнительных библиотек.
* **Linux:** в редких случаях потребуется доустановить системный FFmpeg (`apt install ffmpeg`) — OpenCV использует его для чтения
  видеофайлов.

Проверить, что OpenCV доступен, можно командой `python -c "import cv2; print(cv2.__version__)"`. Если выводится номер версии,
мини-плеер готов к работе.
...
- Кнопка **«Предпросмотр и разметка…»** открывает видеоплеер на OpenCV. Выберите ролик, перемотайте нужный кадр и поправьте координаты — полупрозрачные прямоугольники поверх видео сразу показывают будущее положение delogo. После сохранения значения автоматически переносятся в активный пресет.
- Если OpenCV не найден или видео не удалось открыть, диалог покажет подсказку с командой установки, а координаты можно будет изменить вручную через таблицу.
