...
export const listChromeProfiles = async () => backend?.listChromeProfiles?.();
export const launchChrome = async (payload: { profileName: string; port?: number }) =>
  backend?.launchChrome?.(payload);
export const stopChrome = async (pid: number) => backend?.stopChrome?.(pid);
